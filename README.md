...
Syncing is why I actually bother, and it splits in two. The skills and configuration (`.claude/` and `CLAUDE.md`) live in a GitHub repo that's private to me, which is how the same assistant follows me between my work laptop and the machine at home: pull in the morning, work, then commit and push when I'm done. Teaching Claude something on one computer and having it already known on the other is most of the value.

The notes I keep out of git entirely. Add your vault folder to `.gitignore` and let [Obsidian Sync](https://obsidian.md/sync) carry it instead. Sync is a paid add-on, it's end-to-end encrypted, and it's built for this, whereas a git repo full of your personal notes is both less private and more irritating in practice, since every save wants a commit and two machines editing the same daily note is a merge conflict waiting to happen. Keep the repo private regardless. This kit ships with its example vault committed because you need to be able to clone it in the first place; once the vault is yours and full of your own material, ignore it and let Sync do that half.

If you only ever use one computer, skip all of this. Git is still worth having for the undo history, but nothing in the kit depends on it.
